...
function BetForm({bet,books,holders,tags,settings,onSave,onCancel,sports,isEdit,lastBet,repeatFrom,lastGolfTourney,customTeams,customPlayers,customMarkets,setCustomTeams,setCustomPlayers,setCustomMarkets}){
  ...
  const[multiEntries,setMultiEntries]=useState(initME?initME.map(e=>({id:uid(),book:e.book,holder:e.holder,oddsStr:(!isRepeat&&D.bookEntries)?e.oddsAmer!=null?""+e.oddsAmer:"":"",stakeStr:(!isRepeat&&D.bookEntries)?e.stake!=null?""+e.stake:"":"",toWinStr:(!isRepeat&&D.bookEntries)?e.toWin!=null?""+e.toWin:"":"",riskMode:"risk",isFree:e.isFree||false,freeVal:""})):[{id:uid(),book:dfBook,holder:dfHolder,oddsStr:"",stakeStr:"",toWinStr:"",riskMode:"risk",isFree:false,freeVal:""}]);
  ...
  const handleSave=()=>{
    ...
    const base={betType,isLive,sport,league,selection:fs,marketType:isML?betType:isFut?futCat:marketType,line:line?parseFloat(line):null,participant:isProp?participant:isFut?participant:_wPl?participant:isMatchup?(matchSide==="p2"?matchP2:matchP1):teamName,propStat,overUnder,awayTeam:isFut?undefined:awayTeam,homeTeam:isFut?undefined:homeTeam,betSide,futCat:isFut?futCat:undefined,futDesc:isFut?futDesc:undefined,teaserPts:isTeaser?teaserPts:undefined,matchP1:isMatchup?matchP1:undefined,matchP2:isMatchup?matchP2:undefined,matchSide:isMatchup?matchSide:undefined,tournament:sport==="Golf"?tournament:undefined,oddsAmer,oddsDec,closingOdds:closingOdds?parseInt(closingOdds)||null:null,notes,tags:selTags,placedAt,eventDate:isFut?undefined:eventDate,legs:isML?legs:[{id:uid(),legNumber:1,selection:fs,marketType,line:line?parseFloat(line):null}],result:D.result||null,netProfit:D.netProfit!=null?D.netProfit:null};
    ...
    else if(multiBook&&mT){const be=mT.entries.map(e=>({book:multiEntries.find(m=>m.id===e.id)?.book||"",holder:multiEntries.find(m=>m.id===e.id)?.holder||"",oddsAmer:e.oddsAmer,oddsDec:e.oddsDec,stake:e.stake,toWin:e.toWin,isFree:e.isFree||false}));onSave({...base,book:be.map(e=>e.book).join(", "),holder:be[0]?.holder||holder,oddsAmer:mT.avgAmer,oddsDec:mT.avgDec,stake:mT.totalStake,toWin:mT.totalToWin,potentialPayout:mT.totalStake+mT.totalToWin,bookEntries:be});}
  ...
  return(
    <div style={{paddingTop:20,maxWidth:620,margin:"0 auto"}}>
      ...
      <Sec c="#818cf8" label={"2 - Type"+DOT+"Market"}>
        ...
        <div style={{display:"flex",gap:6}}><button onClick={()=>setIsLive(!isLive)} style={{...BS,background:isLive?"#1a2e1a":"transparent",color:isLive?"#22c55e":"#6b6b8a",border:"1px solid "+(isLive?"#22c55e44":"#2a2a3e")}}>{isLive?"🔴 LIVE":"⏱ Pre"}</button>{!isRR&&<button onClick={()=>{if(!multiBook){setMultiEntries([{id:uid(),book:book,holder:holder,oddsStr:oddsStr,stakeStr:stakeStr,toWinStr:toWinStr,riskMode:riskMode,isFree:selTags.includes("Freebet"),freeVal:freebetVal}]);}setMultiBook(!multiBook);}} style={{...BS,background:multiBook?"#1a1a3e":"transparent",color:multiBook?"#818cf8":"#6b6b8a",border:"1px solid "+(multiBook?"#818cf855":"#2a2a3e")}}>{multiBook?"📚 Multi-Book":"📖 Single"}</button>}</div>
      </Sec>
      {/* 3 */}
